...
def run(
    ...
    for source_index in range(nr_sources):
        ...
        threshold_list.append(
            Threshold(
                threshold_line=Line(threshold_start, threshold_end),
                max_age=cfg.STRONGSORT.MAX_AGE,
                outside_direction=1
            )
        )
    ...
    overlay_plotter = OverlayPlotter(1, names)
    ...
    for frame_idx, (path, im, im0s, vid_cap) in enumerate(dataset):
        ...
        for source_index, detections in enumerate(yolo_detections):  # detections per image
            ...
            if cfg.STRONGSORT.ECC:  # camera motion compensation
                strongsort_list[source_index].tracker.camera_update(prev_frames[source_index], curr_frames[source_index])
            ...
            if detections is not None and len(detections):
                ...
                # Perform tracking
                strong_sort_outputs[source_index] = strongsort_list[source_index].update(boxes_xywh.cpu(),
                                                                                         confidence_scores.cpu(),
                                                                                         classes.cpu(),
                                                                                         im0)

                # draw boxes for visualization
                if len(strong_sort_outputs[source_index]) > 0:
                    for j, (output, confidence_score) in enumerate(zip(strong_sort_outputs[source_index], confidence_scores)):
